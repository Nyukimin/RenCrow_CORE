...
test('To-Be Ops renders exactly five safe summary blocks', () => {
  ...
  const model = buildToBeOpsViewModel({
    advisor: {
      ...
      policy_decisions: [{decision_id: 'policy-1', agent_id: 'shiro', action: 'external_publish', decision: 'allowed', reason: 'DO_NOT_RENDER_POLICY_REASON'}],
    },
  ...
  assert.deepEqual(model.map((block) => block.key), ['advisor-agent', 'knowledge-relation', 'economic-objective', 'policy-decisions', 'recent-trace']);
