...
func TestRunBacklogRunnerStartsActiveItemOnce(t *testing.T) {
	...
	if !ok || observation.Caller != "heartbeat.backlog" || observation.Purpose != "process_backlog_item" || observation.TaskID != agent.lastInput.RootTaskID() {
...
func TestTick_HeartbeatUsesShiroWorkerRoute(t *testing.T) {
	...
	observation, ok := llm.ExecutionObservationFromContext(agent.lastCtx)
	if !ok || observation.TaskID != agent.lastInput.RootTaskID() || observation.TraceID == "" || observation.TraceID != string(agent.lastInput.TraceID()) {
