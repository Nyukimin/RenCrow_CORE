...
func (s *StoryEpisodeService) PrepareToTarget(ctx context.Context) error {
	...
	return s.prepareUntil(ctx, s.store.target)
}
...
func (s *StoryEpisodeService) PrepareAdditional(ctx context.Context, count int) error {
	...
	return s.prepareUntil(ctx, s.store.snapshot().Ready+count)
}
...
次のJSONについて、造語を含む不自然・破損した語、人間関係、時系列、場所、所有関係、世界規則、固有名詞とふりがな、面白さ契約、読み手と合いの手の演出を確認します。
...
codeは lexical_corruption, entity_relation_violation, continuity_violation, world_rule_violation, reading_violation, interest_contract_violation, story_performance_violation, quality_violation のいずれか。
対象JSON:
` + string(payload)
	raw, err := s.generator.Generate(ctx, prompt)
	if err != nil {
