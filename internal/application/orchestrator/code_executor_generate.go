...
func (e *DefaultCodeExecutor) emitCoderGenerateError(req CodeExecutionRequest, target codeTarget, err error) {
	...
	e.emit("agent.response", target.name, "shiro", "エラー: "+err.Error(), route.String(), req.TaskID, sessionID, channel, chatID)
...
func (e *DefaultCodeExecutor) emitCoderGenerateResponse(req CodeExecutionRequest, target codeTarget, response string) {
	...
	sessionID, channel, chatID := turnInputMetadata(req.Input)
	route := req.Input.Route()
	e.emit("agent.response", target.name, "shiro", content, route.String(), req.TaskID, sessionID, channel, chatID)
	e.emit("agent.report", target.name, "shiro", formatAgentHandoffCompletionSpeech("shiro", target.name, content), route.String(), req.TaskID, sessionID, channel, chatID)
	e.emit("agent.response", "shiro", "mio", content, route.String(), req.TaskID, sessionID, channel, chatID)
	e.emit("agent.report", "shiro", "mio", formatShiroToMioReport(route, req.TaskID.String(), content), route.String(), req.TaskID, sessionID, channel, chatID)
}
