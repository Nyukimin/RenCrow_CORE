...
func TestDistributedWorkerTerminalFailurePersistsFailedTaskAndRun(t *testing.T) {
	...
	router := transport.NewMessageRouter()
	defer router.Stop()
	coder := &distMockTransport{response: domaintransport.Message{From: "coder3", To: "mio", Type: domaintransport.MessageTypeResult, Content: "proposal", Proposal: &domaintransport.ProposalPayload{Plan: "edit", Patch: "[]"}}}
	worker := &distMockTransport{response: domaintransport.Message{From: "shiro", To: "mio", Type: domaintransport.MessageTypeResult, Content: "implementation complete\n```go\npackage main\n```", Result: &domaintransport.ResultPayload{Success: false, Summary: "test failed", FailureKind: "test_impact_failed", FailureReason: "provider proposal validation failed", Retryable: false, TestStatus: "failed"}}}
	orch := NewDistributedOrchestrator(&distMockSessionRepo{}, &distMockMioAgent{routeResponse: "CODE3"}, router, session.NewCentralMemory(), map[string]domaintransport.Transport{"coder3": coder, "shiro": worker})
	orch.SetTaskLifecycleManager(owner)
	_, err = orch.ProcessMessage(context.Background(), ProcessMessageRequest{SessionID: "terminal-test", Channel: "viewer", ChatID: "viewer-user", UserMessage: "generate code"})
	if err == nil {
		t.Fatal("worker failure was accepted as route success")
	}
	if !strings.Contains(err.Error(), "provider proposal validation failed") {
		t.Fatalf("owner reason lost: %v", err)
	}
	if len(coder.sentMessages) != 1 || len(worker.sentMessages) != 1 {
		t.Fatalf("terminal failure reran effects: coder=%d worker=%d", len(coder.sentMessages), len(worker.sentMessages))
	}
	...
	if len(tasks) == 0 {
		t.Fatal("no persisted tasks")
	}
	for _, task := range tasks {
		if task.Status != domaintask.StatusFailed {
			t.Fatalf("task %s status=%s", task.TaskID, task.Status)
		}
	}
	...
	if len(runs) == 0 {
		t.Fatal("no persisted runs")
	}
	for _, run := range runs {
		if run.Status != domaintask.RunStatusFailed {
			t.Fatalf("run %s status=%s", run.RunID, run.Status)
		}
	}
	for _, msg := range worker.sentMessages {
		if _, err := owner.Get(context.Background(), msg.TaskID); err != nil {
			t.Fatalf("worker task detached from owner: %v", err)
		}
	}
}
