...
func TestCodeExecutor_CODE1Route(t *testing.T) {
	...
	req := newOrchestratorTestCodeExecutionRequest(t, taskID, "user message", routing.RouteCODE1, "sess-1", "test", "chat-1")
...
func TestCodeExecutor_CODE3_WithProposal(t *testing.T) {
	...
	taskID := modulecore.NewTaskID()
	req := newOrchestratorTestCodeExecutionRequest(t, taskID, "user message", routing.RouteCODE3, "sess-1", "test", "chat-1")
	...
	if workerService.taskID.String() != taskID.String() {
		t.Fatalf("worker taskID=%s, want %s", workerService.taskID.String(), taskID.String())
	}
...
func TestCodeExecutor_CODE2_WithProposal_ExecutesPatch(t *testing.T) {
	...
	taskID := modulecore.NewTaskID()
	req := newOrchestratorTestCodeExecutionRequest(t, taskID, "README.md を更新して", routing.RouteCODE2, "sess-1", "test", "chat-1")
...
func TestCodeExecutor_ModuleRegistryInjectsContextAndWorkspace(t *testing.T) {
	...
	executor := NewDefaultCodeExecutor(nil, coder2, nil, nil, workerService, nil, func(eventType, from, to, content, route, taskID, sessionID, channel, chatID string) {
		events = append(events, codeExecutorEvent{eventType: eventType, from: from, to: to, content: content, route: route, taskID: taskID, sessionID: sessionID, channel: channel, chatID: chatID})
	}).WithModuleResolver(moduleapp.DefaultRegistry())

	taskID := modulecore.NewTaskID()
	req := newOrchestratorTestCodeExecutionRequest(t, taskID, "RenCrow_STT の音声入力を修正して", routing.RouteCODE2, "sess-1", "test", "chat-1")
	...
	for _, event := range events {
		if event.taskID != taskID.String() {
			t.Fatalf("event task ID=%q, want supplied task %q: %+v", event.taskID, taskID, events)
		}
	}
}
...
func TestCodeExecutor_CODE3_WithProposalRecordsSkillChangeEvidence(t *testing.T) {
	...
	taskID := modulecore.NewTaskID()
	req := newOrchestratorTestCodeExecutionRequest(t, taskID, "Skillの挙動を変更して", routing.RouteCODE3, "sess-1", "test", "chat-1")
...
func TestCodeExecutor_CODEDoesNotDynamicSelectCODE3(t *testing.T) {
	...
	taskID := modulecore.NewTaskID()
	req := newOrchestratorTestCodeExecutionRequest(t, taskID, "user message", routing.RouteCODE, "sess-1", "test", "chat-1")
...
func TestCodeExecutor_ProposalPathEmitsLanguageTrace(t *testing.T) {
	...
	taskID := modulecore.NewTaskID()
	req := newOrchestratorTestCodeExecutionRequest(t, taskID, "TTSを直して", routing.RouteCODE3, "viewer", "viewer", "viewer-user")
...
// ヘルパー関数
func noopEventEmitter(eventType, from, to, content, route, taskID, sessionID, channel, chatID string) {
	// テスト用の空実装
}
...
func (m *failingCoderAgent) Generate(ctx context.Context, t conversation.TurnInput, systemPrompt string) (string, error) {
	return "", m.err
}
...
func recordingCodeEventEmitter(events *[]codeExecutorEvent) func(eventType, from, to, content, route, taskID, sessionID, channel, chatID string) {
	return func(eventType, from, to, content, route, taskID, sessionID, channel, chatID string) {
...
func (s *recordingCodeWorkerExecutionService) ExecuteProposal(ctx context.Context, taskID modulecore.TaskID, p *proposal.Proposal) (*patch.PatchExecutionResult, error) {
	s.calls++
	s.taskID = taskID
	s.proposal = p
	if s.err != nil {
...
func (s *recordingCodeWorkerExecutionService) ExecuteProposalInWorkspace(ctx context.Context, taskID modulecore.TaskID, p *proposal.Proposal, workspace string) (*patch.PatchExecutionResult, error) {
	s.workspace = workspace
	return s.ExecuteProposal(ctx, taskID, p)
}
...
func (r *recordingCoderProposalEvidenceRecorder) SaveCoderProposalEvidence(_ context.Context, item domainskill.CoderProposalEvidence) (domainskill.CoderProposalEvidencePaths, error) {
	...
	return domainskill.CoderProposalEvidencePaths{
		RootPath:            "workspace/logs/skill_governance/coder_evidence/task",
		SkillDiffPath:       "workspace/logs/skill_governance/coder_evidence/task/skill_diff.md",
		AgentTranscriptPath: "workspace/logs/skill_governance/coder_evidence/task/agent_transcript.md",
	}, nil
}
