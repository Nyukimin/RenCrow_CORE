...
func TestPhase19DistributedRouteDispatcherCHATBypassesAutonomousExecutor(t *testing.T) {
	...
	dispatcher := newDistributedRouteDispatcher(
		...
		func(ctx context.Context, gotTask conversation.TurnInput, route routing.Route, taskID modulecore.TaskID) (string, error) {
	...
	dispatcher.SetAutonomousExecutor(func(ctx context.Context, t conversation.TurnInput, route routing.Route, taskID modulecore.TaskID, ttsSessionID string) (string, error) {
...
func TestPhase19DistributedRemoteRouteCarriesExactTurnInputProjection(t *testing.T) {
	...
	if sent.TaskID != taskID {
		t.Fatalf("sent task ID=%q, want %q", sent.TaskID, taskID)
	}
