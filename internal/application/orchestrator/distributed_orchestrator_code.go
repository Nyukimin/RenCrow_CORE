...
func (c *distributedCodeExecutionCoordinator) Execute(ctx context.Context, input domainconversation.TurnInput, route routing.Route, taskID modulecore.TaskID) (string, error) {
	...
	log.Printf("[DistributedOrch] code handoff route=%s target=%s task=%s", route, coderAgent, taskIDText)

	shiroWork := fmt.Sprintf("route=%s task=%s のコード作業取りまとめ", route, taskIDText)
	c.emit("agent.delegate", "mio", "shiro", formatAgentHandoffSpeech("mio", "shiro", shiroWork, input.MessageText()), string(route), taskIDText, sessionID, channel, chatID)
	c.emit("agent.acknowledge", "shiro", "mio", formatAgentHandoffReadbackSpeech("mio", "shiro", shiroWork, input.MessageText()), string(route), taskIDText, sessionID, channel, chatID)
	c.emit("agent.start", "mio", "shiro", "コードタスクをShiro経由で実行", string(route), taskIDText, sessionID, channel, chatID)
	c.emitNote("mio", "user", "しろにコード実装の取りまとめをお願いしたよ。", string(route), taskIDText, sessionID, channel, chatID)
	...
	if resolved := c.moduleResolver.Resolve(input.MessageText()); resolved.Found() {
		c.emit("module.selected", "mio", "shiro", resolved.Summary(), string(route), taskIDText, sessionID, channel, chatID)
		requestText = appendModuleContextToCodeRequest(requestText, resolved)
	}

	for attempt := 0; attempt <= c.coderRetryMax(); attempt++ {
		coderWork := fmt.Sprintf("route=%s task=%s retry=%d の設計・コード生成", route, taskIDText, attempt)
		c.emit("agent.delegate", "shiro", coderAgent, formatAgentHandoffSpeech("shiro", coderAgent, coderWork, requestText), string(route), taskIDText, sessionID, channel, chatID)
		c.emit("agent.acknowledge", coderAgent, "shiro", formatAgentHandoffReadbackSpeech("shiro", coderAgent, coderWork, requestText), string(route), taskIDText, sessionID, channel, chatID)
		c.emit("agent.start", "shiro", coderAgent, requestText, string(route), taskIDText, sessionID, channel, chatID)
		if attempt == 0 {
			c.emitNote("shiro", "mio", fmt.Sprintf("%sにコーディング依頼しました。進捗を監視して、必要なら作業を前に進めます。", displayAgentName(coderAgent)), string(route), taskIDText, sessionID, channel, chatID)
		} else {
			c.emit("worker.retry_request", "shiro", coderAgent, fmt.Sprintf("retry=%d", attempt), string(route), taskIDText, sessionID, channel, chatID)
			c.emitNote("shiro", "mio", fmt.Sprintf("%sに修正版patchを再依頼します。retry=%d", displayAgentName(coderAgent), attempt), string(route), taskIDText, sessionID, channel, chatID)
		}

		coderMsg, err := c.buildCoderMessage(coderAgent, requestText, route, input, taskID, attempt)
		if err != nil {
			return "", err
		}
		...
		if err != nil {
			...
			c.emit("agent.report", coderAgent, "shiro", formatAgentHandoffCompletionSpeech("shiro", coderAgent, failureReport), string(route), taskIDText, sessionID, channel, chatID)
			if retryable && attempt < c.coderRetryMax() {
				c.emit("worker.classified_failure", "shiro", coderAgent, fmt.Sprintf("%s: %s", failureKind, reason), string(route), taskIDText, sessionID, channel, chatID)
				requestText = buildCoderRetryInstruction(input.MessageText(), nil, failureKind, reason, attempt+1)
				continue
			}
			c.emit("agent.report", "shiro", "mio", formatAgentHandoffCompletionSpeech("mio", "shiro", failureReport), string(route), taskIDText, sessionID, channel, chatID)
			return "", err
		}
		c.emit("agent.response", coderAgent, "shiro", coderResult.Content, string(route), taskIDText, sessionID, channel, chatID)
		c.emit("agent.report", coderAgent, "shiro", formatAgentHandoffCompletionSpeech("shiro", coderAgent, coderResult.Content), string(route), taskIDText, sessionID, channel, chatID)
		c.emitNote(coderAgent, "shiro", "おわったっす。", string(route), taskIDText, sessionID, channel, chatID)
		c.emitNote("shiro", "mio", fmt.Sprintf("%sの結果を受け取って、内容確認と仕上げを進めます。", displayAgentName(coderAgent)), string(route), taskIDText, sessionID, channel, chatID)

		if coderResult.Proposal == nil {
			return c.finishWithoutProposal(ctx, input, route, taskID, coderAgent, coderResult)
		}
		response, retryReq, retryable, err := c.executeProposal(ctx, input, route, taskID, coderAgent, coderResult, attempt)
		if err != nil {
			return "", err
		}
		if retryable {
	...
	err := fmt.Errorf("coder retry budget exhausted for task %s", taskIDText)
	c.emit("agent.report", "shiro", "mio", formatAgentHandoffCompletionSpeech("mio", "shiro", "実行失敗: "+err.Error()), string(route), taskIDText, sessionID, channel, chatID)
	return "", err
}
...
func (c *distributedCodeExecutionCoordinator) finishWithoutProposal(ctx context.Context, input domainconversation.TurnInput, route routing.Route, taskID modulecore.TaskID, coderAgent string, coderResult domaintransport.Message) (string, error) {
	sessionID, channel, chatID := turnInputMetadata(input)
	taskIDText := taskID.String()
	c.emit("agent.start", "shiro", "mio", "Coder結果をShiroで整形", string(route), taskIDText, sessionID, channel, chatID)
	shiroInput := input.WithMessageText(coderResult.Content).WithRoute(route)
	shiroTask, err := domaintransport.NewTurnInputMessage("mio", "shiro", taskID, shiroInput)
	if err != nil {
...
func (c *distributedCodeExecutionCoordinator) executeProposal(ctx context.Context, input domainconversation.TurnInput, route routing.Route, taskID modulecore.TaskID, coderAgent string, coderResult domaintransport.Message, attempt int) (response, retryRequest string, retryable bool, err error) {
	sessionID, channel, chatID := turnInputMetadata(input)
	taskIDText := taskID.String()
	c.emit("agent.start", "shiro", "mio", "CoderのProposalをWorker実行", string(route), taskIDText, sessionID, channel, chatID)
	execMsg := domaintransport.NewMessage("mio", "shiro", sessionID, taskID, "Execute coder proposal")
	...
	if err != nil {
		...
		if retryableFailure && attempt < c.coderRetryMax() {
			c.emit("worker.classified_failure", "shiro", coderAgent, fmt.Sprintf("%s: %s", failureKind, reason), string(route), taskIDText, sessionID, channel, chatID)
			c.recordCoderProposalEvidence(ctx, input, route, taskID, coderAgent, coderResult.Proposal, domaintransport.Message{}, err)
...
func (c *distributedCodeExecutionCoordinator) recordCoderProposalEvidence(
	...
	if err != nil {
		log.Printf("WARN: failed to save distributed coder proposal evidence task=%s route=%s: %v", taskID.String(), route, err)
		return
	}
	if paths.SkillDiffPath != "" || paths.AgentTranscriptPath != "" {
		log.Printf("Distributed coder proposal evidence saved task=%s skill_diff=%s agent_transcript=%s", taskID.String(), paths.SkillDiffPath, paths.AgentTranscriptPath)
	}
}
