...
func TestExecuteProposal_BlocksSelfServiceRestartBeforeAnyCommandRuns(t *testing.T) {
	...
	_, err := service.ExecuteProposal(context.Background(), task.NewTaskID(), proposal.NewProposal("blocked", patchJSON, "", ""))
	if err == nil {
...
func TestExecuteFileEdit_UpdateRequiresExistingTarget(t *testing.T) {
	...
	_, err := service.ExecuteProposal(context.Background(), task.NewTaskID(), p)
	if err == nil {
...
func TestExecuteGitOperation(t *testing.T) {
	...
	result := svc.executeCommand(context.Background(), task.NewTaskID(), cmd, 0)
...
func TestExecuteGitOperation_CommitAfterAdd(t *testing.T) {
	...
	addResult := svc.executeCommand(context.Background(), task.NewTaskID(), addCmd, 0)
	...
	commitResult := svc.executeCommand(context.Background(), task.NewTaskID(), commitCmd, 1)
