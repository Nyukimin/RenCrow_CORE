...
// GatewayAliasCheck verifies an RenCrow_LLM Gateway alias endpoint.
...
// NewGatewayAliasCheck creates a lightweight health check for RenCrow_LLM Gateway aliases.
func NewGatewayAliasCheck(role, baseURL, model, apiKey string, timeout time.Duration) *GatewayAliasCheck {
	if timeout <= 0 {
		timeout = 3 * time.Second
	}
	return &GatewayAliasCheck{
...
func (c *GatewayAliasCheck) Name() string {
...
func (c *GatewayAliasCheck) Run(ctx context.Context) domainhealth.CheckResult {
	start := time.Now()
	if c.baseURL == "" {
