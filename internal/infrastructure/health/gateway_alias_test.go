...
func TestGatewayAliasCheck_UsesModelsReadinessPath(t *testing.T) {
	...
	check := NewGatewayAliasCheck("Chat", srv.URL, "Chat", "", 0)
...
func TestGatewayAliasCheck_DegradedWhenAliasMissing(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	check := NewGatewayAliasCheck("Worker", srv.URL, "Worker", "", 0)
	result := check.Run(context.Background())
...
func TestGatewayAliasCheck_DownOnBadStatus(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
