...
func TestSQLiteStoreSavesAndListsSuperAgentRecords(t *testing.T) {
	...
	if err != nil || len(queue) != 1 || queue[0].QueueItemID == "" || queue[0].QueueItemID.Validate() != nil {
...
func TestSQLiteRunQueueClaimSurvivesReopenAndRecoversAfterLeaseExpiry(t *testing.T) {
	...
	if completed, err := reopened.CompleteRunQueueItem(context.Background(), string(recovered.QueueItemID), "owner-1", "completed", "stale result", now.Add(62*time.Second)); err != nil || completed {
		t.Fatalf("stale owner completion accepted=%v err=%v", completed, err)
	}
	if completed, err := reopened.CompleteRunQueueItem(context.Background(), string(recovered.QueueItemID), "owner-2", "completed", "resumed", now.Add(78*time.Second)); err != nil || !completed {
