...
func TestGatewayEmbedderEmbedSuccess(t *testing.T) {
	...
	embedder := NewGatewayEmbedderWithOptions("test-key", "Embed", srv.URL, time.Second)
...
func TestGatewayEmbedderEmptyEmbedding(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(map[string]any{"data": []map[string]any{{"embedding": []float64{}}}})
	}))
	defer srv.Close()

	embedder := NewGatewayEmbedderWithOptions("", "Embed", srv.URL, time.Second)
