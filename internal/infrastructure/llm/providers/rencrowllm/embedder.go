...
var _ domconv.EmbeddingProvider = (*GatewayEmbedder)(nil)

// GatewayEmbedder implements EmbeddingProvider against an RenCrow_LLM Gateway-compatible /v1/embeddings endpoint.
...
func NewGatewayEmbedder(apiKey, model string) *GatewayEmbedder {
	return NewGatewayEmbedderWithOptions(apiKey, model, defaultBaseURL, 120*time.Second)
}

func NewGatewayEmbedderWithOptions(apiKey, model, baseURL string, timeout time.Duration) *GatewayEmbedder {
	if baseURL == "" {
