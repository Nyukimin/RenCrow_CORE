...
func (p *GatewayProvider) addThinkingBridgeFields(req map[string]interface{}, streaming bool) {
	modulellm.ApplyThinkingBridgeFields(req, p.thinkingBridge, streaming)
}

func (p *GatewayProvider) addProviderOptions(req map[string]interface{}, options map[string]any) {
	modulellm.ApplyThinkingBridgeProviderOptions(req, p.thinkingBridge, options)
}

func (p *GatewayProvider) addModelContextOption(req map[string]interface{}) {
...
func (p *GatewayProvider) sanitizeThinkingBridgeContent(content, parseStatus, _ string) string {
