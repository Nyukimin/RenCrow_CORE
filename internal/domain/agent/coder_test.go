...
func TestCoderAgentStableRuntimeContextIsAddedWithoutChangingSystemPrompt(t *testing.T) {
	...
	testTask := task.NewTask(task.NewTaskID(), "main.goを作成して", "line", "U123")
...
func TestCoderAgentGenerateWithPromptUsesLightMemory(t *testing.T) {
	...
	_, err := coder.GenerateWithPrompt(context.Background(), task.NewTask(task.NewTaskID(), "second user", "line", "U123"), "coder prompt")
	if err != nil {
		t.Fatalf("GenerateWithPrompt failed: %v", err)
	}
