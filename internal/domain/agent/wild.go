...
func (w *WildAgent) Generate(ctx context.Context, t task.Task) (string, error) {
	...
	if isImageGenerationRequest(userMessage) {
		...
		if w.conversationEngine != nil {
			if commitErr := commitConversationTurn(ctx, w.conversationEngine, t.TaskID().String(), t.ChatID(), userMessage, response, conversation.SpeakerMidori, nil); commitErr != nil {
	...
	if w.conversationEngine != nil {
		pack, err := w.conversationEngine.BeginTurn(ctx, t.ChatID(), userMessage)
		if err != nil {
	...
	if response, ok := exactSharedRecallAnswer(userMessage, recallPack); ok {
		...
		if w.conversationEngine != nil {
			if err := commitConversationTurn(ctx, w.conversationEngine, t.TaskID().String(), t.ChatID(), userMessage, response, conversation.SpeakerMidori, recallPack); err != nil {
				return response, err
			}
		}
		return response, nil
