...
func TestMioMovieCatalogLookupInjectsResultAndSuppressesWebSearch(t *testing.T) {
	...
	_, err := mio.Chat(context.Background(), task.NewTask(task.NewTaskID(), "映画「PERFECT DAYS」の出演者を検索して", "viewer", "user"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestMioMovieCatalogUnavailableSuppressesWebFallback(t *testing.T) {
	...
	mio := NewMioAgent(provider, &mockClassifier{}, &mockRuleDictionary{}, runner, &mockMCPClient{}, nil)
	_, err := mio.Chat(context.Background(), task.NewTask(task.NewTaskID(), "役所広司の出演映画を検索して", "viewer", "user"))
	if err != nil {
		t.Fatal(err)
	}
	if webCalls != 0 {
...
func TestMioMovieCatalogAllSemanticIntentsUseCatalogWithoutWeb(t *testing.T) {
	...
	for _, request := range requests {
		t.Run(request, func(t *testing.T) {
			...
			if _, err := mio.Chat(context.Background(), task.NewTask(task.NewTaskID(), request, "viewer", "user")); err != nil {
				t.Fatal(err)
			}
