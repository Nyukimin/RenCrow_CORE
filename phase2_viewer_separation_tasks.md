...
> [WARN]
> 本書は2026-06-12時点の調査・計画記録であり、現行の実装状況またはbacklogではない。
> 行数、優先度、分離候補を現在判断へ流用せず、製品仕様は`docs/README.md`から選んだ
> `docs/01`〜`09`、実装状態はproduction wiringとtestを確認する。
> 具体的な変更隔離・再利用・テスタビリティの利得がない一括分離は再開しない。
