...
type voiceDirectFinalHandler interface {
	...
	NotifyVoiceDirectFirstToken(ctx context.Context, req orchestrator.ProcessVoiceDirectRequest, jobID task.TaskID, firstTokenAt time.Time)
}
...
func (a voiceDirectBridgeAdapter) NotifyVoiceDirectFirstToken(ctx context.Context, req orchestrator.ProcessVoiceDirectRequest, jobID task.TaskID, firstTokenAt time.Time) {
	if a.handler == nil {
		return
	}
