...
func runJobsCommand(args []string, manager jobCommandStore, out io.Writer, errOut io.Writer) int {
	...
	case "show":
		if len(args) < 2 || strings.TrimSpace(args[1]) == "" {
			fmt.Fprintln(errOut, "usage: rencrow jobs show <job_id>")
			return 1
		}
	...
	case "start":
		if len(args) < 2 || strings.TrimSpace(args[1]) == "" {
			fmt.Fprintln(errOut, "usage: rencrow jobs start <job_id>")
			return 1
		}
	...
	case "status":
		if len(args) < 3 || strings.TrimSpace(args[1]) == "" || strings.TrimSpace(args[2]) == "" {
			fmt.Fprintln(errOut, "usage: rencrow jobs status <job_id> <status> [--summary text]")
			return 1
		}
	...
	case "cancel":
		if len(args) < 2 || strings.TrimSpace(args[1]) == "" {
			fmt.Fprintln(errOut, "usage: rencrow jobs cancel <job_id> [--summary text]")
			return 1
		}
	...
	default:
		...
		fmt.Fprintln(errOut, "usage: rencrow jobs [list|show|create|start|status|cancel|notifications]")
		return 1
	}
}
