...
func runOllamaCommand(
	...
	default:
		...
		fmt.Fprintln(errOut, "usage: rencrow ollama [status|restart]")
		return 1
	}
}
...
func buildOllamaRestartAction(cfg *config.Config) (string, func() error, error) {
	...
	restartCmd := strings.TrimSpace(os.Getenv("RENCROW_OLLAMA_RESTART_CMD"))
	if restartCmd == "" {
	...
	sshUser := strings.TrimSpace(os.Getenv("RENCROW_OLLAMA_SSH_USER"))
	if sshUser == "" {
		sshUser = strings.TrimSpace(os.Getenv("USER"))
	}
	if sshUser == "" {
		return "", nil, fmt.Errorf("RENCROW_OLLAMA_SSH_USER is required for remote ollama restart")
	}

	sshArgs := []string{fmt.Sprintf("%s@%s", sshUser, host), restartCmd}
	if keyPath := strings.TrimSpace(os.Getenv("RENCROW_OLLAMA_SSH_KEY_PATH")); keyPath != "" {
