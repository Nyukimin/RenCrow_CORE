...
func resolveVoiceChatWebSocketHandler(plan modulevoicechat.BridgePlan, inputAudio voiceChatInputAudioSettings, voiceDirect voiceDirectFinalHandler, idleNotifier orchestrator.IdleNotifier, owners voiceChatExecutionOwners) http.Handler {
	switch {
	case plan.Disabled:
	...
	default:
		if voiceChatHTTPBaseURLFromGateway(plan.GatewayURL) != "" {
			return handleVoiceChatInputAudioBridge(plan.GatewayURL, inputAudio, voiceDirect, idleNotifier, owners)
		}
