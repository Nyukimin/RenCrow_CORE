...
func buildOrchestratorRuntime(
	...
	if cfg.Distributed.Enabled {
		...
		startSuperAgentRunQueueScheduler(cfg, deps.superAgentStore, deps.distOrch, deps.taskManager, newBackgroundJobFailureReporter(deps.eventRelay, deps.taskManager))
		return
	}
	...
	startSuperAgentRunQueueScheduler(cfg, deps.superAgentStore, orch, deps.taskManager, newBackgroundJobFailureReporter(deps.eventRelay, deps.taskManager))
}
...
Key facts:
- This directory IS the RenCrow Go codebase (github.com/Nyukimin/RenCrow_CORE)
