...
func TestBuildPrimaryLLMProvidersIgnoresDisabledGatewayAndLegacyLocalLLM(t *testing.T) {
	var models []string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var body map[string]any
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
	...
	cfg := &config.Config{
		...
		LocalLLM: config.LocalLLMConfig{
			Enabled: true, BaseURL: "http://physical-endpoint.invalid", ChatModel: "physical-model",
		},
	}
	providers := buildPrimaryLLMProviders(cfg, nil)
