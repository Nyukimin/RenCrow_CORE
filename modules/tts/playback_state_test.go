...
func TestBuildPlaybackStateHealthReport(t *testing.T) {
	...
	if live.Metadata["pending_session_count"] != 1 || live.Metadata["pending_public_playback_count"] != 2 {
...
func TestBuildPendingPlaybackSnapshotSortsAndCopiesIDs(t *testing.T) {
	...
	if got.PendingSessionCount != 2 || got.PendingPublicPlaybackCount != 2 || got.TopicGateCount != 3 || got.TopicRouteCount != 4 {
		t.Fatalf("unexpected counts: %+v", got)
	}
	if got.PendingSessionIDs[0] != "session-a" || got.PendingPublicPlaybackRefs[0] != "response-a" {
		t.Fatalf("ids were not sorted: %+v", got)
	}

	sessions[0] = "mutated"
	responses[0] = "mutated"
	if got.PendingSessionIDs[1] != "session-b" || got.PendingPublicPlaybackRefs[1] != "response-b" {
...
func TestBuildPublicPlaybackSnapshotClampsNegativeCounts(t *testing.T) {
	...
	if got.RouteCount != 0 || got.StaleRouteCount != 2 || got.NextChunkSessionCount != 0 || got.NextPublicPlaybackSessionCount != 4 {
...
func TestBuildPlaybackStateSnapshotCombinesPendingAndPublicState(t *testing.T) {
	pending := PendingPlaybackSnapshot{
		...
		PendingPublicPlaybackRefs:  []string{"r1", "r2"},
		TopicGateCount:             3,
		TopicRouteCount:            4,
	}
	public := PublicPlaybackSnapshot{
		...
		NextPublicPlaybackSessionCount: 8,
	}

	got := BuildPlaybackStateSnapshot(pending, public)
	if got.PendingSessionCount != 1 || got.PendingPublicPlaybackCount != 2 || got.TopicGateCount != 3 || got.TopicRouteCount != 4 {
		t.Fatalf("pending state was not mapped: %+v", got)
	}
	if got.PublicRouteCount != 5 || got.PublicStaleRouteCount != 6 || got.NextChunkSessionCount != 7 || got.NextPublicPlaybackSessionCount != 8 {
