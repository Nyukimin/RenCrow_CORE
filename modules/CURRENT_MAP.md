...
- `modules/tts` owns Gateway request, playback, session, and event rules; `cmd/rencrow` constructs only the RenCrow_TTS Gateway provider.
...
- CORE no longer owns physical TTS provider priority or Irodori/SBV2 adapters. `cmd/rencrow` and `internal/infrastructure/tts` construct and call only the RenCrow_TTS Gateway.
...
- RenCrow_TTS Gateway request payload construction, emotion voice fallback, and request ID header construction live in `modules/tts`; `internal/infrastructure/tts` keeps HTTP calls and validates the Gateway marker.
