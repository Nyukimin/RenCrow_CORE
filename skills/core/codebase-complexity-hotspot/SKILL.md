...
この Skill は最適化 patch を自動適用するためのものではない。証拠、リスク、必要テスト、次アクションを整理し、修正する場合は別 Goal と同期policy判定に分離する。
...
- cache、DB query rewrite、concurrency、認証、課金、削除処理は同期policy判定、rollback evidence、実行後検証を必須とする。
