...
**作成日**: 2025-12-11
**最終更新**: 2026-03-07
**バージョン**: 1.2 (命名規則ファースト追加、デバッグ原則強化)
...
5. **発見は、共有可能な形にする。**
   うまくいった操作、調査手順、プロンプト、検証観点、失敗から得た教訓は、会話だけで消費せず、必要に応じて再利用可能な形で残す。
   共有単位は重い資料ではなく、短い説明、実際に使ったプロンプト、コマンド、スクリーンショット、チェックリスト、runbook、rule、skill などの小さい資産を優先する。

6. **説明だけでなく、使える実例で答える。**
   「どうやったか」を聞かれたら、抽象説明だけでなく、実際に使えるプロンプト、コマンド、差分、検証手順、失敗時の見方を添える。
   ただし、重要な制約や安全判断はプロンプト任せにせず、コード、テスト、ログ、ルールへ落とす。

7. **導入支援は、日常作業の倍率にする。**
   AI 活用・ツール運用・プロンプト改善は、別仕事として肥大化させず、通常作業の中で自然に共有できる軽い習慣にする。
   週次メモ、定期スレッド、短い「今日わかったこと」、よく使う skill / prompt の共有など、常用者が増える導線を作る。
...
7. **【情報収集ファースト — 仮説は後】**
   問題が発生したら、仮説を立てる前に利用可能な全ての診断情報を収集する。
   ログ、stderr/stdout、プロセス状態、実行コマンド、設定ファイル — 見える情報を先に全て集める。
   症状だけ見て原因を憶測せず、「何が観測できるか」を最優先する。

8. **【リモート実行失敗の鉄則 — 相手側のログ確認】**
   SSH/RPC/HTTP などリモート実行でエラーが起きたら、必ず相手側の stderr/stdout/ログファイルを最初に確認する。
   ネットワークやライブラリの問題だと決めつける前に、リモートプロセスが何をエラーとして出力しているかを見る。
   手元の症状（EOF、timeout など）は結果であり、原因はリモート側にあることが多い。

9. **【試行は2回まで — 解決しなければ情報収集に戻る】**
   同じアプローチの微修正を繰り返さない。2回試して解決しなければ、別の仮説を試すのではなく、情報収集が不足していると考えて調査に戻る。
   - 1回目: 仮説Aを試す → 失敗
   - 2回目: 仮説Aを修正して再試行 → 失敗
   - **3回目以降: 情報収集に戻る**（仮説Bを試すのではなく、見落としている情報源を探す）
   それでも分からなければユーザーに相談する。

10. **【命名規則を最初に決める — 実装より先に】**
    新しい概念・識別子（Agent名、コマンド名、設定キーなど）を導入する前に、命名規則を決めて文書化する。
    複数箇所で使う識別子（config、CLI引数、ログ、コード）は全箇所で完全に統一する。
    大文字/小文字の混在、類似名（`code`/`coder`）、微妙な差異（`Nyukimin`/`Nyukimi`）は禁止。
    **AIは文字列の1文字差を見分けるのが苦手**なため、人間が設計段階で排除する。

    **悪い例（実例）**:
    - config: `Coder3`, CLI: `--agent coder3`, ログ: `Coder3` の混在
    - 結果: 4回のトライアンドエラー、根本原因の見落とし、stderr確認まで気づかず
...
13. **【本質対策を優先し、暫定対策で完了にしない】**
    目の前の障害を止血する暫定対策は許可するが、同一ターンで恒久対策（不正状態を作れない設計）を必ず提示する。
    可能ならそのまま恒久対策まで実装し、完了判定は恒久対策ベースで行う。

14. **【Castで逃がさず、入力不正は設計で防ぐ】**
    型変換や実行時の辻褄合わせで不正データを通さない。
    「本来入れてはいけない値」が入る余地は、生成契約・コンストラクタ検証・Noop実装・Fail-Fast などで境界で遮断する。

15. **【フォールバックで隠蔽しない。障害は明示する】**
    フォールバックは「処理継続のためのごまかし」に使わない。
    異常時は、失敗した事実・失敗箇所・原因カテゴリを明示エラーとして返し、観測可能な状態にする。

16. **【`raw`ログは無加工・無圧縮】**
    `raw` と名付けるログは、受信・生成した文字列を一切加工せずそのまま記録する。
    trim、truncate、省略記号（`...`）付与、要約、整形、マスクを禁止する。
    長さ制御が必要な場合は `raw` とは別に `preview` などの別キーを追加し、`raw` 自体は常に完全値を保持する。

17. **【プロンプトよりコードを優先する】**
    分岐、検証、正規化、ID付与、ルーティング、表示/音声の出し分け、禁止事項の強制など、実装コードで決定的に書ける制御をプロンプトへ逃がさない。
    プロンプトは、LLMにしかできない自然言語生成・要約・分類の候補生成に限定する。
    コードとプロンプトが矛盾する場合は常にコードを正とし、プロンプト側を削るか補助情報へ降格する。
    「プロンプトに書いてあるから守られる」は完了条件にしない。重要な制約はコード、テスト、ログで検証可能にする。

18. **【相対判断は文脈基準で行う】**
    「長い/短い」「重い/軽い」「速い/遅い」「十分/不十分」「普通/異常」などの相対判断を、一般論だけで決めない。
    まず対象ドメイン、ユーザー観測、正本仕様、実データ、同種ケースの基準を確認する。
    基準が未確認なら断定せず、「未確認の仮説」として扱い、測定・比較・再現確認へ進む。
    相対判断を理由に回避策へ逃げる前に、対象仕様として本当に許容できるのかを確認する。
...
- コード、設定、API、UI、runtime の挙動変更は **TDD（Red-Green-Refactor）** を必須とする
- テストは FIRST / AAA の原則を意識して書く
- 重要なビジネスロジックにはユニットテストを必ず付ける
- 利用者フロー、複数境界、実ブラウザに影響する変更は E2E テストを必須とする

詳細ルール（適用判断、シナリオ行列、隔離、完了証跡等）は
...
- 調査メモ・分析レポートはGit管理外の `logs/investigations/` 以下に保存し、採用した結論だけを現行正本へ反映する
