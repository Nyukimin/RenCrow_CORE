...
製品仕様は`docs/README.md`に列挙された文書だけを参照します。不足がある場合は、実装事実を確認して該当文書を更新します。別の正本を作りません。
...
secretをsource、config sample、test、docs、logへ埋め込みません。破壊的操作、外部side effect、同期実行policyの契約は `docs/07_安全・自動実行・データ方針.md` に従います。
