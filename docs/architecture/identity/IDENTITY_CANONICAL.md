...
- IdleChatの保存済みcheckpointからの後継発行は`StartRunFromCheckpoint`へ期待する直前RunID、
  ...
  時計を補正して履歴順を捏造しない。
- `ExecuteRunEffect`は同期的な外部生成呼出しの前に、Task ownerの短い読取transaction内で
  Task／Run／Actor／writer generationを照合し、store所有の同じTaskの排他fenceを呼出しが戻るまで保持する。
  fenceは同じTaskのRun差替えと終端更新を排他する一方、外部callbackの間もglobal batch lockを保持せず、別Taskの更新を妨げない。
  callback内からTask ownerを再呼出し、Run完了処理、切り離された非同期処理を行わない。
  fenceのadmissionとcallbackは取消と期限を守る。process終了後に外部へ残った処理の停止や、遠隔systemのexactly-onceを保証したとは扱わない。
- Failure / Problem: `ExecuteRunEffect`が外部callback中もJSONLのglobal batch lockを保持し、別Taskの終端保存を期限切れにした。
  Causeは外部効果と永続化lockの境界が逆転していたことであり、LessonはTask単位の排他をglobal lockから分離すること。
  Invariantは同じTaskの効果と更新を直列化し、別Taskは進行でき、active fence中にraw bulkが触れたTaskのcommitを拒否し、Closeはadmitted fenceをdrainしてからwriter leaseを解放すること。
  Enforcementはstore所有のcontext-cancellable Task fence、commit前のtouched Task検査、Closeのadmission／drainで行う。
  Testsは別Taskの完了、同じTaskの待機・取消・直列化、stale identity、raw bulk bypass拒否、Close中の新writer拒否とactive callback待機を検証する。
- Failure / Problem: 中断された生成の終端処理が、同じTask／Run履歴の反復decodeによってcleanup期限を超え、実行中の行を残した。
  Causeは同じtransaction内の読取・更新・commit検証で履歴全体を再読していたことであり、Lessonはtransactionに閉じた検証済みsnapshotを再利用すること。
  Invariantは既存JSONLを唯一の正本とし、全履歴の遷移検証、pending appendの即時反映、owner照合を維持し、snapshotを次transactionへ持ち越さないこと。
  Enforcementは共通の履歴fold、immutableなcommitted view、pending版数、返却値のcopy、lock待機後と返却前のcontext確認で行う。cleanup期限は延長しない。
  Testsは返却値の変更からの隔離、pending状態の反映、rollback後の再読、履歴遷移の不正拒否、race検査で保証する。実データ相当の性能と実際の終了・再起動は別途receiptで検証する。
- Failure / Problem: IdleChatの終端処理が、同じTaskのGetTask／ListRunsを別transactionで先に読むため、5秒のcleanup期限内にcanonicalな終端更新へ到達できないことがあった。
  Causeはownerの`CompleteRun`が持つ一つのTask transaction内のTask／Run／Actor／generation／active検査と、呼出側の履歴preflightが重複していたことである。Lessonは呼出側がexact RunのActorと状態だけを読み、履歴の最新性・曖昧性・別active検査と終端更新をownerへ集約すること。
  Invariantは`CompleteRun`が全Task Run履歴を同じtransaction snapshotで検証し、要求Runが一意な最新Runでない場合、同時刻の最新候補がある場合、または別active Runがある場合に一切更新せず拒否すること。既に終端済みの再試行は`VerifyRunCompletion`のread transactionで同じ履歴検証を行い、cleanup期限は延長しない。
  Enforcementはtaskmanagerの共通latest-history validatorと、IdleChatのvalidated `GetRun`一回の読取境界で行う。Testsはlate／ambiguous／other-active／identity／actor／stale-generationを拒否し、completion前のTask／ListRuns preflightがないこととterminal verificationのidempotenceを検証する。
- Failure / Problem: clean restart後、保存済みWord checkpointが旧writer generationのRunning Runを指し、completion maintenanceが`run writer generation is stale`を繰り返して後続生成を恒久的に遮断した。
  Causeは旧Actor callbackを拒否する通常`CompleteRun`だけを再起動後のdurable completionにも使用し、新processが取得したwriter leaseを根拠に旧Runを収束させるowner境界がなかったことである。Lessonは実行中callbackのadmissionと、旧processに属するdurable checkpointの回復を同じgeneration判定へ畳み込まないこと。
  Invariantは新processが前writerのOS lock解放後に取得した単調増加generationを持ち、domain ownerが旧process由来の外部効果を新process内に保持していないと証明できる場合だけ、同一Task、同一Actor、一意な最新かつactiveな旧generation Runを指定statusへ原子的に閉じること。current／zero／future generation、別Task／Actor、曖昧履歴、終端Runを置換せず、後継Runを同時発行しない。
  EnforcementはTask ownerの`RecoverRunAfterProcessRestart`と、providerを呼ばないIdleChat Word／Forecast／Dialogue completion maintenanceだけに置く。Testsはwriter再取得後のexact旧Run収束、current Run no-op、RunID／writer generation不変、Taskとの同時waiting化、各producerのrestart回復とprovider非呼出しを検証する。
- Failure / Problem: 認証済み`/viewer/idlechat/stop`が能動Dialogue生成の終了前に成功を返し、取消後もTask／Runを`running`に残した。
  Causeは`StopManualMode`が会話状態とprovider contextだけを中断し、既にadmitされたgeneration workのjoinと保存済みcompletion intentの終端処理を行わなかったことである。Lessonは利用者に見える停止応答を、内部stateの非active化だけで完了させないこと。
  Invariantは能動停止が新しいgeneration workのadmissionを一時的に閉じ、admit済みworkのTask／Run cleanupまでjoinし、Word／Forecast／Dialogueのpending completionをbounded cleanup contextで処理してから返すこと。通常の能動停止後だけadmissionを再開し、process shutdownで取消済みのownerは再開しない。
  Enforcementは`StopManualMode`のclose admission→interrupt→join→completion retry→live-owner reopen順と、process `Stop`と共有するcompletion helperで行う。Testsは遅延Dialogue providerの取消中に停止が返らず、解放後にconversationとDialogueのexact Runが非runningとなり、process停止ではadmissionが閉じたままになることを検証する。
...
- Forecast、Story、Dialogueの生成も同じ保存→Run終端→checkpoint削除の順序を使う。生成と意味評価はLLM、保存・再開・Run状態・再生許可はCORE ownerが決定する。永続保存先または正規Run ownerがない場合は、生成前に失敗する。
- Dialogueは生成前にRunと生成入力をcheckpointへ保存する。保存済みのturn、受理済みprefix、validationを再開に使い、Run完了やcleanupだけの再試行で本文を再生成しない。意味検証に最終失敗したartifactは失敗証跡として保存し、Runを成功にしない。
- DialogueのRun終端要求は、生成時のTaskID／RunIDを親に持つcheckpointの`pending_completion`へ、要求するTask終端status、summary、reasonとして保存する。次のDialogue生成admissionとStop後のflushはこのintentを同じTask／Runへprovider呼出しなしで再試行し、保存・照合に失敗した意図を捨てない。intentを正常に処理した後もwaitingのcheckpointは保持し、succeeded／failedのcheckpointだけをcanonicalな終端照合と、同じTask／Run・revisionに一致するdurable artifactのready／failed証明後に削除する。
- Wordの生成admissionは、保持されたsingle／doubleのcheckpointが指すexact Running Task／Runを、checkpoint resumeやprovider呼出しより先にcanonical ownerで終端化する。保存済みstockが同じTask／Runを証明するときだけsucceeded、それ以外は理由付きwaitingとし、終端失敗時はcheckpointと同じpairを保持して後継Runやproviderを発行しない。Stopはgeneration workのjoin後に同じpairだけをprovider・stock変更なしで再試行し、waiting checkpointを保持する。
- Word／Forecast topicのcompletion maintenanceはLLM refillとauto-chatのadmission gateから独立し、各monitor tickで既存のtopic producer single-flight gate内から保持されたexact Running Task／Runを同期的に再試行する。producerが稼働中ならそのpairを触らず次回へ回し、matching stockがあるときだけsucceeded、公開済み結果がないときは理由付きwaiting、checkpoint／stockのidentity mismatchまたはnil・pathなし・load failureはprovider・後継Run・stock変更の前にfail closedする。Stopはgeneration workのjoin後に同じbounded aggregateでWordとForecastを再試行し、成功したterminal closureの後だけcheckpointを削除する。
- Storyの既存作品タイトル補完・タイトル修復・suffix修復も同じTask ownerとcheckpointを使う。終端済みRunからは発行前にrerun intentを保存し、同一Task・assigneeの`explicit_rerun`だけを採用する。修復checkpointの`source_run_id`は元作品のRun参照として固定し、後継ID保存直後の停止を未発行と混同しない。中断後は`checkpoint_resume`で再開し、生成済みsuffix、タイトル、意味検査結果を再利用する。受理済みprefixとMessageIDは維持し、Run変更時にrevisionを進める。修復元Runと保存済みEpisodeの対応、変更禁止field、stageと完了phaseの一致を検証する。新revisionの保存は同じstore lock内で最新の再生回数・最終再生時刻を引き継ぎ、古いcheckpointで巻き戻さない。
- Storyのready artifactとForecast stockは、同じRunのcheckpointが残る間は再生対象にしない。Storyはcheckpoint内のEpisodeIDも照合し、後継Runへの切替から新revision保存まで旧revisionを公開しない。保存済みartifactからの復旧は正規Task ownerのexact Runを確認し、別Runや欠落した成果物を成功と推定しない。sourceでの故障注入試験は、配備後の実Actor利用・再起動・Viewer traceの代わりにはならない。
...
cohort生成中もwriter停止を維持するproduction手順では、capture後にserviceを開始せずruntime maskだけを
解除し、systemdが公開する固定unitのExecStart、inactive、MainPID zeroとinstalled runtime／active configを
owner CLIが再検証してから再度runtime maskし、port 18790 listener zeroを確認する。この停止状態だけを
受理する。cohort生成のため旧runtimeを一度再開する手順は、startup writerによるactive driftを生むため
使用しない。mask中はsystemdのunit identityが`/dev/null`へ隠れるため、その状態を所有証明として受理しない。
...
### Step 13 follow-up: BrowserTrace APIArtifactの正準IdentityとKind

- BrowserTraceの`APIArtifact`は、正準`ArtifactID`（`modules/core`のtyped `ArtifactID`、wireは`artifact_id`）と`ArtifactKind`（wireは`artifact_kind`）を保持する。既存の`artifact_type`はcontent roleとして残し、正準Artifact分類は`ArtifactKind`だけが表す。両者を同一名で兼用しない。
- content roleから`ArtifactKind`への対応は`internal/domain/browsertrace/artifact_kind.go`の`ArtifactKindForAPIArtifactType`／`ValidateAPIArtifactKindForType`が唯一の宣言であり、builder、store、rencrowclientへ写像を複製しない。未知のcontent roleはfallbackを持たずerrorとし、読み出し時のKind remintも行わない。永続化時の検証は保存されたKindをcontent roleと突き合わせ、上書きしない。
- 生成側（`BuildAPIArtifactsWithValidations`）はKind解決に失敗した場合errorを返し、Viewer handlerは500で終えて保存を継続しない。Kind欠落のまま保存されたArtifactを正準Artifactとして扱わない。Workstreamへの連携は既存のstring境界を維持する。
- **Failure / Problem:** BrowserTraceのbuilderは`NewArtifactID()`で正準IDを生成していたが、`APIArtifact`の`ArtifactID`が`string`で保存validatorも非空だけを検査したため、`art_` prefixを持たないopaqueな`artifact_id`を受理し、`ArtifactKind`自体を持っていなかった。そのため永続化された過去rowとtest fixturesに旧形式が残っていた。Cause: content roleを表す`artifact_type`と正準Artifact分類を同一オブジェクトで兼用しながら、正準ID検証とKind宣言をdomain境界に持たせていなかった。
- **Lesson / Invariant:** Artifactとして永続化する対象は、正準ID検証とKind宣言をdomainが所有する境界を通す。Kindはcontent roleから決定的に解決し、推測や既定値で埋めない。
- **Enforcement / Tests:** Kind解決の単一宣言、未知content roleの拒否、非正準`artifact_id`の拒否、UUIDv5／v7の受理、JSONへの`artifact_kind`保持、SQLite／JSONL roundtrip、rencrowclient投影のtype・content・owner identity保持をowner testsで検査する。
- BrowserTraceの`APICoverageReport`は単一の本文stringを持たないため、content hashの入力はbrowsertrace domainが単一宣言する本文投影のbyte列に限る。compact JSONで`observed_flows`→`observed_endpoints`→`missing_flows`→`recommended_next_traces`の固定順に並べ、nilと空listは空配列へ正規化して同一digestとして扱う。`artifact_id`・`artifact_kind`・`task_id`・`run_id`・`actor_id`・`created_at`・`content_hash`自身・`superseded_by`は入力へ含めず、同一本文なら別`artifact_id`の行も同一digestを持つため、remintや後継参照の追加・更新は本文digestを変えない。摘要形式と後継`ArtifactID`検査は`modules/core`の`ContentHashOf`／`ValidateContentHash`／`ValidateArtifactSupersession`をそのまま再利用し、投影の知識だけをbrowsertraceが所有する。
- SuperAgentの`ContextPack`も単一の本文stringを持たないため、content hashの入力はsuperagent domainが単一宣言する本文投影のbyte列に限る。compact JSONで`summary`→`included_sources`→`token_estimate`の固定順に並べ、nilと空listは空配列へ正規化して同一digestとして扱う（`internal/domain/superagent/contextpack_content.go`が唯一の宣言）。`token_estimate`はproducerが`summary`と同時に保存する保存済みコンテキストのtoken見積りなので本文に含める。`artifact_id`・`artifact_kind`・`task_id`・`run_id`・`workstream_id`・`created_at`・`content_hash`自身・`superseded_by`は本文digestへ含まず、remintと後継参照の変更はdigestを変えない。摘要形式と後継ID検査は`modules/core`の`ContentHashOf`／`ValidateContentHash`／`ValidateArtifactSupersession`を再利用し、読み出し時の自動補完は行わない。
- **Boundary / 実施記録:** 2026-09-21にQwen3.8-flash-next（worker）がsource実装とreview修正、該当5 packageのGo検査を実施し、監督Astraが修正後の同一5 packageを独立実行して全てexit 0を確認した（`Tmp/test-runtime/_qwen-supervision/astra_source_unit_independent.log`、2026-09-21 11:55 UTC）。workerの局所検査とAstraの独立再実行は別実行者によるものである。Content hashとsupersessionも同一日にsource実装し、形式の単一宣言は`modules/core/artifact_content.go`（`ContentHashOf`は永続Contentの実byte列を正規化せず`sha256:`＋lowercase 64hexへ摘要。`ValidateArtifactSupersession`は任意の後継`ArtifactID`参照を共有validatorで検査し自己参照を拒否）、生成とfield使用は`internal/domain/browsertrace`が所有する。読出し時の再計算・自動remintは行わない。`modules/core`を含む6 packageのAstra独立実行はexit 0で実行中のsource変更は無かった（`Tmp/test-runtime/_qwen-supervision/astra_hash_source_independent.json`、2026-09-21 13:38:45-13:39:19 UTC）。live DBの`api_artifact`は2026-09-17の記録で旧形式、2026-09-21 12:41 UTCのread-only inventoryで6 rows・`artifact_kind` 0件・全件が旧opaque IDと実測した（`Tmp/test-runtime/_qwen-supervision/s13_api_artifact_inventory.log`）。既存rowへのremint移行、binary配備、CORE再起動、正規Viewer route実測は未実施であり、本項のsource検証はそれを代替しない。Step 13全体は`partial`のままである。`APICoverageReport`へのcontent hash／supersession適用（IU-18c1）は2026-09-21にsource実装・両store往復・rencrowclient投影・fixture更新まで完了し、Qwen3.8-flash-nextの局所検査（赤=`s13_coverage_hash_red.log` exit 1の7失敗と`s13_coverage_nil_empty_red.log`のnil／空listの実差、緑=domain／persistence／modules/core fullとfocused producer・client契約・viewer handler exit 0）と、監督Astraの独立実行を別実行者の証拠として区別して保持する。Astraの14:38:46-14:39:07 UTCの6 package exit 0（`astra_coverage_source_independent.json`）は実行中にワーカーの`gofmt -w`で3 fileが変化したため受入根拠にせず、編集停止後の14:45:19-14:46:10 UTCに同一HEAD `f159ed9`でpersistence・client・viewer full、Windows/amd64 CGO無効の関連6 package `go vet`、`go build ./...`までexit 0かつ`source_changed_during_test`なしを再実行して受入した（`astra_coverage_stable.json`）。worker側はloopback listenが`socket: operation not permitted`となりapplication／viewer packageのfull実行はFAILするため、6 package完走の証跡はAstra側のみである。native Windows実行ではなく、live DBのremint・binary配備・CORE再起動・正規Viewer route実測は未実施。`ContextPack`へのcontent hash／supersession適用（IU-18c2）は2026-09-21にsource実装を完了し、producer（`internal/application/orchestrator/superagent_runtime.go`の保存直前で本文から摘要をstamp）、SQLite／JSONL往復、rencrowclient投影（DTO全体をdomainへstruct変換して渡され、client側で本文fieldを再列挙しない）まで接続した。Qwen3.8-flash-nextの局所検査（赤=`s13_cp_domain_red.log` exit 1の4失敗、緑=domain／persistence superagent full exit 0とclient契約focused・producer focused exit 0、`s13_cp_vet_build.log`の`go vet`／`go build ./...` exit 0）とは別に、監督Astraの独立実行を受入根拠とする（`astra_cp_source.json`と各.log、2026-09-21 15:30:05-15:31:08 UTC、同一HEAD `f159ed9`で5 package full・Windows/amd64 CGO無効の関連5 package `go vet`／`go build ./...`が全てexit 0、`source_changed_during_test`なし）。live `context_pack` rowへの`content_hash`付与、CORE再起動、正規Viewer route実測、native Windows実行は未実施である。`DailyRoutineReport`／`ChannelDraft`へのcontent hash／supersession適用（IU-18c3）は2026-09-21にsource実装を完了し、本文投影の単一宣言は`internal/domain/revenue/artifact_content.go`（reportは10 field、draftは`channel`→`subject`→`body`のcompact JSON固定順、nil listは同所で空listへ正規化して欠落と空を同一digest、`status`と`external_send_applied`はpolicy／lifecycle状態なので本文digestへ非混入）、producerは`internal/domain/revenue/validation.go`の`BuildDailyRoutineReport`が最終本文から摘要をstampし、viewerのreport／draft createはhash未指定のときだけstampして空白等の不正な非空入力は黙って補正せずstoreの拒否へ返す。SQLite／JSONLは両fieldを完全一致で往復し、非空後継IDの保持と不正hashの書込み拒否をassertする（`suggested_actions`は要素単位比較、新依存なし）。rencrowclientはDTO全体をdomain型へstruct変換して本文fieldを再列挙しない。Qwen3.8-flash-nextの局所検査（緑＝`s13_revenue_store_green3.log` exit 0、client契約focused exit 0、`s13_revenue_vet_fmt.log`のgofmt offenders 0と`go vet` 4 package exit 0／client fullは`socket: operation not permitted`で未完走＝環境失敗）とは別に、監督Astraの独立実行を受入根拠とする（`astra_revenue_stable.json`と各.log、2026-09-21 16:54:50-16:55:22 UTCで`internal/infrastructure/persistence/revenue`／`pkg/rencrowclient` full、Windows/amd64 CGO無効の関連6 package `go vet`、`go build ./...`が全てexit 0、`source_changed_during_test`なし）。live revenue rowへの`content_hash`付与、CORE再起動、正規Viewer route実測、native Windows実行は未実施である。`VerificationReport` への content hash／supersession（IU-18c4）は 2026-09-21 に source 実装を完了し、本文投影の単一宣言は `internal/domain/verification/artifact_content.go`（`status`→`claim_count`→`verified_count`→`weak_count`→`unsupported_count`→`conflict_count`→`not_checked_count`→`claims`→`questions`→`evidence`→`skip_reason` の固定宣言順、nil list と nested `Claim.Evidence` は同所で空 list へ正規化して同一 digest、identity metadata と Save 失敗注釈の `error_kind`／`error` は非混入）、producer は `internal/application/verification/pipeline.go` が確定本文から摘要を stamp し、本文摘要の計算が失敗したときは `Save` を呼ばず error を返す（Save 0 件を test で assert）。保存は `internal/infrastructure/persistence/verification/report_store.go` の JSONLReportStore が両 field を完全一致で往復し、hash 欠落・他本文 digest・形式不正・自己参照後継の書込み拒否と正規 v5／v7 後継の受理を assert、公開は `internal/adapter/viewer/verification_handler.go` が domain 直接 JSON encoder で `content_hash`／`superseded_by` を出力する（`rencrowclient` に VerificationReport DTO は実在しないため新設せず、drift は Viewer handler test で確認）。Qwen3.8-flash-next の局所検査（赤＝`s13_verification_domain_red2.log`・`s13_verification_producer_red.log` exit 1、緑＝`s13_verification_domain_green2.log`・`s13_verification_producer_green.log`・`s13_verification_store_green.log` exit 0、`-run Verification` focused exit 0、`s13_verification_gofmt.log` offenders 0）とは別に、監督Astra の独立実行を受入根拠とする（domain exit 0＝`astra_verification_domain.json` 17:34:19-17:34:20 UTC、接続 5 package full・Windows/amd64 CGO 無効の vet／`go build ./...` exit 0＝`astra_verification_source.json` 17:50:36-17:51:27 UTC、`source_changed_during_test` なし）。live verification report row への `content_hash` 付与、CORE 再起動、正規 Viewer route 実測、native Windows 実行は未実施である。IU-18d のうち browsertrace SQLite owner の `SupersedeAPIArtifact` と通常 `SaveAPIArtifact` の迂回封じ込めは source 実装完了（stage1-4、2026-09-21 20:15-21:18 UTC に Qwen3.8-flash-next が persistence／domain browsertrace full・`go vet`・`go build ./...` の exit 0 を実測、`Tmp/test-runtime/_qwen-supervision/s13_supersede_stage4a*.log`〜`s13_supersede_stage4c4_*.log`。予約 `sql.Conn` ＋ BEGIN IMMEDIATE の 1 transaction で読み・照合・書込みを閉じ、scope（TaskID/RunID/ActorID/WorkstreamID と Type・Kind）不一致・循環・別後継・不正 hash を無変換で拒否、同一後継 retry は冪等、跨 instance 競合は成功 1 件、rollback は live な期限付き ctx で実行。摘要形式と後継検査は `modules/core/artifact_content.go` を再利用）。JSONL APIArtifact の WAL foundation（`NewJSONLStore` が `(*JSONLStore, error)` を返し、空root既定を維持して `jsonlbatch.New`（保持fileは `api_artifact.jsonl` のみ）→`Recover`→lock下で load＋`ValidateAPIArtifact`、失敗時 store を渡さない。`SaveAPIArtifact` は `batch.Write` の追記payloadで確定、`ListAPIArtifacts` は `batch.Read` の lock内で private load を呼び同一 artifact_id の最終出現のみを新着優先（最終出現昇順→reverseLimit 1回）で返し、履歴全bodyを保持しない。reader は 64KiB初期／16MiB上限の境界付き bufferで 64KiB超recordを受理し超過は fail-closed、constructor 後の file 欠落は error。呼出元 `cmd/rencrow/runtime_dependencies.go` と呼び出し側testは error をそのまま扱い fallback を作らない。2026-09-21 22:15 UTC に Qwen が focused test 3件 exit 0（`Tmp/test-runtime/_qwen-supervision/s13_jsonl_focused_red.log`＝constructor破損拒否／latest順／≈100KiB往復）、persistence/browsertrace full exit 1 は既知の意図的Red `TestJSONLStoreSaveAPIArtifactRejectsNewRowWithSuccessor` のみ（`s13_jsonl_store_full.log` 22:15:28 UTC）、`go vet ./internal/infrastructure/persistence/browsertrace ./cmd/rencrow` exit 0（`s13_jsonl_vet.log`）、`go build ./...` exit 0・gofmt offenders 0 を実測。当時は独立実行待ち。その後、通常Save guard・全writer同一lock下のSupersede・共有chain・並行/cancel/WAL recoveryまでQwenが実装し、2026-09-22 00:13:45–00:15:28 UTCにAstra独立5check全exit0・source hash差分0でsource受入した。証跡=`Tmp/test-runtime/_qwen-supervision/astra_jsonl_supersede_source.json`）。残件は 実 supersede 事実（`artifact.superseded`）の durable 公開意図と canonical Event durable receipt・正規 Viewer route 接続（作成事実 `artifact.created` の durable intent と再起動 drain は 2026-09-22 に source 実装済み、次項）、Workstream 本文 owner、live 配備・再起動後 route 実測、live `api_artifact` 6 rows（2026-09-21 12:41 UTC 実測）と live verification report row への `content_hash` 付与を伴う remint 移行、実 supersede 操作の運用受入、live verification report に対する正規 Viewer route 観測、Step 14 依存の Verification Report artifact 作業（check plan の result_consumer 側）である。Verification Report の ArtifactKind は `internal/domain/verification/types.go` に実装済み（Kind=report を Validate で必須）で、content_hash／superseded_by の本文 digest 接続（IU-18c4）も source 実装・Astra 独立受入済み（domain exit 0＝`Tmp/test-runtime/_qwen-supervision/astra_verification_domain.json` 17:34:19-17:34:20 UTC、接続 5 package full・Windows vet・build exit 0＝`astra_verification_source.json` 17:50:36-17:51:27 UTC）なので、Kind 未実装として数え直さない。2026-09-21 21:23:48–21:27:01 UTC、AstraがIU-18d SQLite source単位を独立受入。modules/core・domain/browsertrace・persistence/browsertrace・application/browsertrace・viewer・rencrowclientの全test、Windows/amd64同6package vet・全build、store並行／cancel／cleanupのfocused -raceは全exit0。Go/go.mod/go.sum実行前後hash差分0。証跡=Tmp/test-runtime/_qwen-supervision/astra_sqlite_supersede_source.json。Qwenの実装・局所検査とは別実行者。native Windows実行・JSONL・Event／正規route・他Artifact owner・live移行／配備・Step13全体は未完。
- BrowserTrace `APIArtifact` の**作成事実（`artifact.created`）を正準 Event へ公開する durable intent** は 2026-09-22 に Qwen3.8-flash-next が source 実装した。本文だけ先んに永続化され正準 Event が永久に欠落するクラッシュ窓を、owner ローカル intent の原子的保存で閉じる（第 2 の EventStore・汎用 framework・RequestID dedup・新 index は不作）。domain 単一宣言は `internal/domain/browsertrace/publication.go`（`APIArtifactCreatedEventType="artifact.created"`、component `browsertrace`、`APIArtifactPublicationActorKind="agent"`、payload は `artifact_kind`／`artifact_type`／`content_hash` の 3 key のみ）。検査は作成時の `ValidateAPIArtifactPublicationIntent`（本文 digest 一致・後継 edge 無し必須）と、保存済み intent 専用の `ValidatePersistedAPIArtifactPublicationIntent`（正準 envelope と payload、不変 identity／scope 参照のみ検証し、後続の本文更新や後継 edge 追加を不正として起動失敗化しない）へ分離する。保存 API は両 store 共通で `CreateAPIArtifactWithPublicationIntent`／`ListAPIArtifactPublicationIntents`：SQLite は `sqlite_publication_intent.go` の `api_artifact_publication_intent`（`artifact_id` PK・`event_id` UNIQUE）を既存 `withArtifactTransaction` の同一 BEGIN IMMEDIATE transaction に書き、同一 artifact＋完全一致 intent の retry のみ no-op、intent を持たない既存 row は移行補完を暗黙に行わず error。JSONL は `jsonl_publication_intent.go` の `api_artifact_publication.jsonl` を同一 `jsonlbatch` WAL に登録し（受理済み artifact reader の「全行が APIArtifact」不変を保持）、`artifact_id`／`event_id` の重複矛盾を last-row-wins で無く拒否する。List は `artifact_id` の排他 keyset 昇順・page 既定 50／上限 1000 clamp・総量 1000 打ち切り無しで、lock 解放後に呼出側が EventStore I/O を行う。公開は `internal/application/browsertrace/publication.go` が `NewAPIArtifactCreationIntent`（正準 v7 EventID を新規採番・`EventSeq=0`）、`PublishAPIArtifactCreationFacts`（canonical `GetByID` → 欠落時 `AppendSequenced` → `EventSeq` を除いた readback 一致、競合は `ErrAPIArtifactPublicationConflict`）、`PublishPendingAPIArtifactCreationFacts`（全 intent を keyset で枯れるまで drain）を所有し、公開先 iface は `GetByID`／`AppendSequenced` の 2 方法に限る。runtime は `cmd/rencrow/runtime_browser_trace_recovery.go` を `cmd/rencrow/runtime_dependencies.go:1231`（両 store open 後・route 起動前、30s 期限、失敗は WARN で起動を止めない）から呼び、再起動後は利用者再送に依らず明示復旧で drain する。Viewer の fetcher proposal handler は検証済み candidate の保存と同時に作成事実を公開し、非正準 Workstream は書込み前に拒否する。Qwen 局所実測＝focused fetcher 5 test exit 0（`Tmp/test-runtime/_qwen-supervision/s13_fetcher_proposal_creation_fact.log`）、domain／persistence／jsonlbatch full exit 0（`s13_step13_src_packages.log`）、`go build ./...` exit 0、gofmt offenders 0・`go vet ./internal/adapter/viewer ./cmd/rencrow` exit 0（`s13_fetcher_proposal_vet_gofmt.log` 03:58:37Z）。sandbox は loopback listen が `socket: operation not permitted` のため `httptest.NewServer` を使う viewer test が panic し、socket 必須 test を `-skip` した full 完走 exit 0 13.271s（`s13_fetcher_proposal_viewer_full_nosocket.log` 04:12:15Z）が Qwen 側の上限で、socket test を含む full 完走と application package（同種の既存 2 test を `-skip` で exit 0＝`s13_app_creation_fact_green.log`）の完走証跡は Astra 側の独立実行を要する。本単位の Astra 独立実行は 2026-09-22 04:15 UTC 時点で未完であり、実 supersede 事実の永続化・`/supersede` route・実 route 観測・live remint／配備／native Windows を代替しない。
