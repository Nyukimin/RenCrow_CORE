...
初期投入と更新は `rencrow knowledge index-wiki` で行う。

```bash
GOCACHE=/tmp/rencrow-gocache go run ./cmd/rencrow knowledge index-wiki docs/wiki --repo-root . --json
```
...
```bash
GOCACHE=/tmp/rencrow-gocache go test ./internal/domain/conversation ./internal/infrastructure/persistence/conversation
```
...
- `rencrow knowledge index-wiki` が Wiki page を L1 SQLite へ投入できる。
