...
CLI は `cmd/rencrow/main.go` の command switch に追加する。既存パターンは `source-registry`、`knowledge`、`evidence` である。
...
`cmd/rencrow/main.go` に `web-gather` command を追加する。`cmdWebGather()` は `source-registry` CLI と同じく config を読み、`Conversation.L1SQLitePath` から L1 store を初期化する。
...
```bash
rencrow web-gather url <url> [options]
```
...
```bash
rencrow web-gather search <query> --provider searxng [options]
rencrow web-gather search-and-fetch <query> --provider searxng [options]
rencrow web-gather run-source <source_id> [--json]
rencrow web-gather webwright-fetch --task <task> [--start-url <url>] [--task-id <id>] [--dry-run]
rencrow web-gather import-webwright-jsonl <path> [--json]
...
```bash
GOCACHE=/tmp/rencrow-gocache go test ./modules/webgather
```
...
```bash
GOCACHE=/tmp/rencrow-gocache go test ./internal/application/webgather
```
...
```bash
GOCACHE=/tmp/rencrow-gocache go test ./internal/infrastructure/webgather
```
...
```bash
GOCACHE=/tmp/rencrow-gocache go test ./cmd/rencrow -run WebGather
```
...
```bash
GOCACHE=/tmp/rencrow-gocache go test ./internal/infrastructure/persistence/conversation ./internal/adapter/viewer
```
...
```bash
rencrow web-gather url https://example.com --namespace kb:web --json
...
7. `cmd/rencrow/cli_web_gather.go` を追加する。
...
- `rencrow web-gather url <url>` が direct URL を fetch / extract / pending staging できる
