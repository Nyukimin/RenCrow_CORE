...
RenCrow workspace root
  RenCrow_CORE  Core / Chat / Viewer / runtime
...
`RenCrow_CORE/AGENTS.md` に残すべき内容:
...
| `RenCrow_CORE/modules/stt/**` | gateway integration、viewer chat input boundary |
| `RenCrow_CORE/viewer/**` | desktop / mobile browser verification、pointer-events、z-index |
| `RenCrow_CORE/rencrow-data/**` | snapshot_id、paper trade audit、approval metadata |
| `RenCrow_CORE/systemd/**` | user service install、restart、WorkingDirectory |
| `RenCrow_CORE/internal/infrastructure/persistence/**` | DB / JSONL migration and audit boundary |
...
| Skill | 目的 |
| --- | --- |
| `rencrow-service-rebuild-restart` | `make install`、`rencrow.service` restart、health / viewer verification |
...
| `understand-rencrow-graph` | Understand Anything graph generation / dashboard launch |
