...
あなたは RenCrow / RenCrow_CORE の LLM音声高速化オーケストレーターです。
...
- `cmd/rencrow/voice_chat_runtime_bridge_test.go`
...
- rencrow が `session.commit` を受けた時刻
- rencrow が RenCrow_LLM へ `session.commit` を送った時刻
- RenCrow_LLM が `llm.final` を生成した時刻
- rencrow が `llm.final` を受けた時刻
...
```bash
GOCACHE=/tmp/rencrow-gocache go test ./cmd/rencrow ./modules/voicechat
node --test internal/adapter/viewer/viewer_vds_https.test.mjs
GOCACHE=/tmp/rencrow-gocache go build ./cmd/rencrow
```
...
```bash
tail -n 220 /home/nyukimi/.rencrow/logs/rencrow.log | rg -n "voice-chat|session.commit|llm.final|llm.delta|error|relay"
journalctl --user -u rencrow.service --since "10 minutes ago" --no-pager | rg -n "voice-chat|session.commit|llm.final|llm.delta|error|relay"
```
...
install -m 755 ./rencrow /home/nyukimi/.local/bin/rencrow
...
1. rencrow bridgeで `session.commit` の実受信・転送時刻をログ/metrics化する
2. RenCrow_LLM側の `llm.delta` 粒度をまとめる、または `final` 優先送信にする
3. rencrowで `llm.delta` をさらに集約し、`llm.final` を優先転送する
...
- rencrowがRenCrow_LLMから受けた `llm.final` をViewerへ送る前にdelta処理で詰まっている
...
```text
あなたは RenCrow / RenCrow_CORE の軽量コーダーです。
...
| service反映済み | `/health` と `systemctl --user status rencrow.service` |
