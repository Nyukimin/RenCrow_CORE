...
- `cmd/rencrow/runtime_idlechat.go:50-53`
- `cmd/rencrow/runtime_idlechat.go:114-149`
...
- `GOCACHE=/tmp/rencrow-go-cache go test ./internal/application/idlechat`
- `GOCACHE=/tmp/rencrow-go-cache go test ./cmd/rencrow`
- `git diff --check`

## 2026-05-27 live smoke

`df8319e` 反映後に `make install`、`rencrow.service` 再起動を行い、起動ログで Forecast provider policy の反映を確認した。
