...
2.  モデル常駐確認
    -   chat-v1 が loaded
    -   worker-v1 が loaded
    -   **新規（2026-02-24）**: モデルの `context_length` が制約を満たすか確認
        -   `MaxContext`: 8192 を超える context_length は NG（大きすぎる num_ctx によるロード失敗を防止）
        -   `MinContext`: 必要に応じて最小値も設定可能
3.  応答時間監視
    -   応答 p95 を測定（例: 2秒超で警告）
4.  連続失敗回数
    -   3回連続失敗で異常判定
...
1.  Ubuntu側で異常検知
    -   Ollama サーバー到達不可
    -   必要なモデルが未ロード
    -   **新規（2026-02-24）**: モデルの context_length が制約違反
2.  Slack/LINEへ通知
3.  Windows復旧トリガー実行
    -   `ollama_restart_command` 実行（設定されている場合）
    -   warmupスクリプト再実行
4.  再確認
    -   モデルロード状況確認
    -   context_length 制約確認
