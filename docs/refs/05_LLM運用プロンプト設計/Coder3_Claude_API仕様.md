md_content = """# Coder3 仕様（Claude API 専用・RenCrow用）

作成日: 2026-02-24  
対象: RenCrow / OpenClaw 運用における「Coder3」  
目的: 高品質コーディング/推論をClaudeで行う。ただし **OAuthトークンやブラウザセッションは使わず**、**APIキー経由**に限定し、RenCrowの「承認フロー」と整合させる。

---

## 0. 前提（決定事項）
- OAuthトークンは使わない（決定）
- RenCrowはユーザー承認を求めるタイプの自動化を行う（決定）
...
- 保存先はRenCrowの標準ログ/Obsidian等に合わせる（別仕様に従う）
