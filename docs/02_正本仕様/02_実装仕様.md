...
- 辞書定義の履歴ソースは元`docs.zip`内`03_旧分割仕様アーカイブ/実装仕様_02.md`とする。通常入口には存在しないため、変更時は一時復帰して現行code・testとの差分を監査し、採用内容を本書へ吸収する
...
### 4.3 WorkerInput
- 入力スキーマの履歴ソースは元`docs.zip`内`03_旧分割仕様アーカイブ/実装仕様_03.md`とする。通常入口には存在しないため、変更時は一時復帰して現行code・testとの差分を監査し、採用内容を本書へ吸収する
