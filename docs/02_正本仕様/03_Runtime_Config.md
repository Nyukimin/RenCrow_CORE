...
RenCrow から Codex を呼ぶ場合は、Codex 側の MCP server を直接制御するのではなく、picoclaw の Worker ToolRunner に `codex.run` を登録し、Codex CLI の非対話実行 `codex exec` を呼ぶ。
...
- `working_dir`: Codex を実行する git repository root。空なら picoclaw の `self_source_dir` を使う。
