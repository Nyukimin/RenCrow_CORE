...
この文書は、`RenCrow_CORE` 現ブランチの push 済み HEAD を `RenCrow_CORE` Ver0.80 の seed / staging source として扱い、新規 Public repository へ投入する前に満たす条件を固定する。
...
| 項目 | 方針 |
| --- | --- |
| Source repo | `RenCrow_CORE` 現ブランチ |
...
- `docs/02_正本仕様/05_RenCrow_CORE_Ver0.80_モジュール構成仕様.md`、`06_RenCrow_CORE_Ver0.80_モジュール化実装仕様.md`、`07_RenCrow_CORE_Ver0.80_組み換え実装作業資料.md`、この文書。
...
旧 `RenCrow_CORE`、旧 PR、旧 v3 branch、旧 docs path を前提にした README 文章は、RenCrow_CORE 起点 README では使わない。
...
1. `RenCrow_CORE` 現ブランチで Ver0.80 の構成変更を完了する。
...
このファイルは削除指示ではない。`RenCrow_CORE` staging repo では既存 fixture、runtime 設定、検証 artifact、local tool binary が残っていてもよいが、新規 Public repository `RenCrow_CORE` へ投入する snapshot からは除外する。
...
Ver0.80 seed では Go module path は `github.com/Nyukimin/RenCrow_CORE` のまま保持する。Public repository 名を `RenCrow_CORE` に変えることと、Go module path を変更することは別作業である。module path rename は全 import path と downstream 利用者に影響するため、Ver0.80 初期投入では行わない。

## Public repo 公開後の次フェーズ判断

Public repo `RenCrow_CORE` の初期投入後は、次を Ver0.80 maintenance として扱う。

- CI は Public repo 側に別 commit で追加する。初期 gate は clean clone で実証済みの `go test ./modules/...` と `go test ./cmd/rencrow ./internal/features/... ./internal/adapter/viewer ./modules/...` に限定する。
