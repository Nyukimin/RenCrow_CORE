...
この README と `01` から `10` の仕様書は、`main` における RenCrow_CORE の唯一の現行正本です。利用者、連携モジュール開発者、contributor が同じ現在仕様を参照できるよう、採用済みの製品契約だけを置きます。内部の実装手順、解析データ、引き継ぎ資料、旧仕様、未採用案は含めません。
