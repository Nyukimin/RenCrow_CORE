...
`q`（statement／Evidenceの3–256 rune部分一致）と`offset`（0以上1,000,000以下）を受け、`items | total=null | limit | offset | has_more`を返します。
通常表示のたびに大容量DBを全件COUNTせず、`limit+1`のbounded queryから`has_more`を決定します。
検索は正本L1 eventとtrigger同期する再生成可能な`l1_user_memory_viewer_projection`とFTS5 trigram索引から
bounded DTOを読み、UserMemory schema／state／Evidenceを再検証して返します。入力はFTS構文ではなくphraseとしてescapeします。
...
`progress`のread-only queryは、writeを1接続へ直列化する通常L1 poolと、lifecycle／ProfilePromotion context用の
2接続read poolの双方から分離した1接続のSQLite `query_only` handleで同一WAL snapshotを読みます。
background writeや複数のbackground readのpool待ちをAPI timeoutへ持ち込まず、このhandleによる状態変更は拒否します。
`retry`／`finalize --apply`は引き続きowner write transactionだけを使います。
...
session JSONの通常runtime読取では`task_id`だけを受理し、旧`job_id`を含むhistory項目は拒否します。旧値の変換はproduction
migrationだけが行い、移行namespaceは`6570d821-e63e-592d-a51f-8cf4b43cdba5`、UUIDv5 nameは
...
- Game turnのLLM requestはnon-stream textで実行する。COREは先頭のaction identifier
  token（`A-Za-z0-9_.-`）だけを決定的に切り出し、`available_actions`の一要素との
...
| `GET /viewer/llm-ops/node/models?node_id=` | 1ノードのModel Fit Assessment一覧。任意query: `limit`（1..200、省略時は`llm_capability.llmfit.top_limit`）、`use_case`（general／coding／reasoning／chat／multimodal／embedding）、`runtime`（any／mlx／llamacpp／vllm／bitnetcpp）、`min_fit`（perfect／good／marginal／too_tight）、`max_context`（正の整数）。許可値以外は400 |
...
- 各ノードの`status`は`online`（最新取得成功）／`stale`（直近値を保持し、最新取得またはhealth checkが失敗）／
  `offline`（保持値なし）／`disabled`（`llm_capability.llmfit.enabled: false`）。`collected_at`はRFC3339またはnull、
