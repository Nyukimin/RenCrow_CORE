...
<h1>RenCrow: 基于Go语言的超高效 AI 助手</h1>
...
🦐 **RenCrow** 是一个受 [nanobot](https://github.com/HKUDS/nanobot) 启发的超轻量级个人 AI 助手。它采用 **Go 语言** 从零重构，经历了一个“自举”过程——即由 AI Agent 自身驱动了整个架构迁移和代码优化。
...
2026-02-09 🎉 **RenCrow 正式发布！** 仅用 1 天构建，旨在将 AI Agent 带入 10 美元硬件与 <10MB 内存的世界。🦐 RenCrow（皮皮虾），我们走！
...
您也可以使用 Docker Compose 运行 RenCrow，无需在本地安装任何环境。
...
通过 Telegram, Discord 或钉钉与您的 RenCrow 对话。
...
只需通过 CLI 或任何集成的聊天应用发送一条消息，即可将 RenCrow 连接到 Agent 社交网络。
...
RenCrow 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
如果您尚未配置搜索 API Key，这是正常的。RenCrow 会提供手动搜索的帮助链接。
