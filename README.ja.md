<div align="center">
<img src="assets/logo.jpg" alt="RenCrow" width="512">

<h1>RenCrow: Go で書かれた超効率 AI アシスタント</h1>
...
🦐 RenCrow は [nanobot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。
...
## 📢 ニュース
2026-02-09 🎉 RenCrow リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 皮皮虾，我们走！
...
### 🐜 革新的な省フットプリントデプロイ
RenCrow はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/sipeed/rencrow/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに RenCrow を実行できます。
...
```bash
# 質問を投げる
docker compose run --rm rencrow-agent -m "What is 2+2?"
...
> [!TIP]
> `~/.rencrow/config.json` に API キーを設定してください。
...
Telegram、Discord、QQ、DingTalk、LINE で RenCrow と会話できます
...
> 画像・音声・動画・ファイル（PDF/文書など）は LINE Content API 経由で一時ダウンロードされ、RenCrow の添付情報として処理されます。

> **Docker Compose**: `rencrow-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。

</details>

## ⚙️ 設定

設定ファイル: `~/.rencrow/config.json`

### ワークスペース構成

RenCrow は設定されたワークスペース（デフォルト: `~/.rencrow/workspace`）にデータを保存します：
...
RenCrow はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.rencrow/workspace` | エージェントの作業ディレクトリ |
...
**方法2: 環境変数**
```bash
export RENCROW_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
RenCrow は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `RENCROW_HEARTBEAT_ENABLED=false` で無効化
- `RENCROW_HEARTBEAT_INTERVAL=60` で間隔変更
...
| コマンド | 説明 |
|---------|------|
| `rencrow onboard` | 設定＆ワークスペースの初期化 |
| `rencrow agent -m "..."` | エージェントとチャット |
| `rencrow agent` | インタラクティブチャットモード |
| `rencrow gateway` | ゲートウェイを起動 |
| `rencrow status` | ステータスを表示 |
...
<img src="assets/wechat.png" alt="RenCrow" width="512">
...
別のインスタンスが実行中の場合に発生します。`rencrow gateway` が 1 つだけ実行されていることを確認してください。
